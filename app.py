...
if menu == "Entenda os dados":
    ...
    if st.button("Buscar"):
        filt = resultados[resultados['enrollee_id'] == id_input]
        if not filt.empty:
            prob = filt['probability'].values[0]
            st.write(f"A probabilidade do candidato de ID {id_input} estar em busca de um novo emprego é: {prob*100:.2f}")
        else:
