...
if menu == "Entenda os dados":
    ...
    st.markdown("""<div style='text-align: justify;'><h5>Neste projeto, temos por objetivo determinar se um candidato a uma vaga de cientista de dados será propenso a trocar de emprego após realizar o treinamento oferecido pela empresa ou não. Como é do interesse das empresas evitar a contratação de candidatos que as deixarão após o treinamento, podemos tratar esse problema como um caso de <b>Classificação Binária</b>. Ou seja, um caso em que o <i>target</i> é binário: False (não deixará a empresa) ou True (deixará a empresa).<br>
        ...
        Outliers foram considerados como valores numéricos acima de 1 amplitude interquartílica do terceiro quartil ou abaixo de 1 amplitude interquartílica do primeiro quartil.<br>
        As colunas numéricas nominais foram escaladas utilizando o <i>StandardScaler</i>.<br>
        Como foi identificado um forte desbalanceamento nas classes do <i>target</i> — com aproximadamente 75% de dados False e 25% True — quatro técnicas de balanceamento foram testadas: SMOTE, ADASYN, Random Over-Sampling e Random Under-Sampling.<br>
        Os modelos de classificação avaliados inicialmente foram: <i>LogisticRegression</i>, <i>RandomForestClassifier</i> e <i>XGBClassifier</i>, testados em uma gama de hiperparâmetros definida por meio do <i>GridSearchCV</i>.<br>
        Variáveis constantes foram removidas com base em sua variância, utilizando o <i>VarianceThreshold</i>, e as 10 variáveis mais relevantes foram selecionadas por meio do <i>SelectKBest</i>.<br>
        O <b>recall</b> foi adotado como métrica principal de avaliação dos modelos, pois, de acordo com os objetivos do problema, é mais importante identificar o maior número possível de funcionários propensos a pedir demissão (targets True) do que prever corretamente os casos negativos.<br>
        ...
        O código utilizado pode ser encontrado no <a href='https://github.com/seurepositorio' target='_blank'>repositório do GitHub</a>.</h5></div>""", unsafe_allow_html=True)
