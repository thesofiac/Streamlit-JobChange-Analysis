...
if menu == "Entenda os dados":
    ...
    st.markdown("<h3>Cerca de <span style='color:#E07A5F;'>25%</span> dos candidatos estão em busca de um novo emprego</h3>", unsafe_allow_html=True)
    st.markdown("E candidatos <span style='color:#E07A5F;'>sem experiência prévia</span> relevante, procuram por um emprego **<span style='color:#E07A5F;'com mais frequência</span>**.")
    
    st.markdown("## As intenções dos candidatos **<span style='color:#E07A5F;'não</span>** são afetadas por:")
    st.markdown("- Gênero\n- Área de formação\n- Horas de treinamento")

    with col1:
        st.markdown("#### Candidatos que saíram de seus empregos há mais tempo têm menor chance de estarem procurando por um novo emprego")

        anos = ['1', '2', '3', '4', '>4', 'Nunca trabalhou']
        porcentagens = [26, 24, 23, 22, 18, 18]
        fig, ax = plt.subplots()
        ax.bar(anos, porcentagens)
        ax.set_ylabel('Porcentagem de candidatos')
        ax.set_title('Anos desde o último emprego')
        st.pyplot(fig)


    # Bloco 2: Escolaridade
    with col2:
        st.markdown("#### Candidatos graduados buscam mais por empregos")
        escolaridade = ['Fundamental', 'Médio', 'Graduação', 'Mestrado', 'Doutorado']
        porcentagens2 = [13, 20, 28, 21, 14]
        fig2, ax2 = plt.subplots()
        ax2.bar(escolaridade, porcentagens2, color='orange')
        ax2.set_ylabel('Porcentagem de candidatos')
        ax2.set_title('Escolaridade')
        st.pyplot(fig2)

    # Bloco 3: IDH
    st.markdown("### Quanto maior o IDH da cidade, **menos candidatos** buscam novos empregos")
    idh_labels = ['Baixo', 'Médio', 'Alto']
    idh_values = [59, 49, 17]
    fig3, ax3 = plt.subplots()
    ax3.bar(idh_labels, idh_values, color='green')
    ax3.set_ylabel('Porcentagem de candidatos')
    ax3.set_title('Busca por emprego x IDH')
    st.pyplot(fig3)
