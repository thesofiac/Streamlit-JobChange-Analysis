...
if menu == "Entenda os dados":
    ...
    if st.button("Prever"):
        ...
        st.write(f"A probabilidade do candidato estar em busca de um novo emprego é: {prob*100:.0f}%")
    ...
    if st.button("Buscar"):
        ...
        if not filt.empty:
            prob = filt['Probabilidade'].values[0]
            st.write(f"A probabilidade do candidato de ID {id_input} estar em busca de um novo emprego é: {prob*100:.0f}%")
        else:
